...
export const CREATURE_SIZE_INFO: Record<CreatureSize, {
...
export const TERRAIN_INFO: Record<TerrainType, {
  ...
  Empty: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 0, description: 'Open terrain' },
  Wall: { passable: false, blocksLineOfSight: true, movementCost: 0, coverBonus: 0, description: 'Solid wall' },
  DifficultTerrain: { passable: true, blocksLineOfSight: false, movementCost: 2, coverBonus: 0, description: 'Difficult terrain (costs extra movement)' },
  Water: { passable: true, blocksLineOfSight: false, movementCost: 2, coverBonus: 0, description: 'Water (may require swimming)' },
  Pit: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 0, description: 'Pit or hazardous terrain' },
  Door: { passable: false, blocksLineOfSight: true, movementCost: 0, coverBonus: 0, description: 'Door (can be opened)' },
  Window: { passable: false, blocksLineOfSight: false, movementCost: 0, coverBonus: 0, description: 'Window (blocks movement, not sight)' },
  Cover: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 2, description: 'Half cover (+2 AC/Dex saves)' },
  HeavyCover: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 5, description: 'Three-quarters cover (+5 AC/Dex saves)' },
  Stairs: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 0, description: 'Stairs to different level' },
  Hazard: { passable: true, blocksLineOfSight: false, movementCost: 1, coverBonus: 0, description: 'Hazardous terrain' }
};
...
export type BattleCommand =
  ...
  | { type: 'MOVE_CREATURE'; data: { creatureId: string; position: GridPosition } }
  | { type: 'NEXT_TURN'; data: {} }
  | { type: 'START_BATTLE'; data: {} }
  | { type: 'UNDO'; data: {} }
  | { type: 'CREATE_BATTLE'; data: { name: string; mode?: BattleMode; mapSize?: { width: number; height: number }; sceneDescription?: string } }
  | { type: 'SET_TERRAIN'; data: { positions: GridPosition[]; terrain: TerrainType; doorOpen?: boolean; elevation?: number; hazardDamage?: number } }
...
export type CampaignCommand =
  ...
  | { type: 'UPDATE_CAMPAIGN'; data: { campaignId: string; updates: { name?: string; description?: string } } }
  | { type: 'DELETE_CAMPAIGN'; data: { campaignId: string } }
  | { type: 'ADD_CREATURE_FROM_CAMPAIGN'; data: { battleId: string; campaignCreatureId: string; position?: GridPosition } }
  | { type: 'MOVE_CREATURE_TO_CAMPAIGN'; data: { creatureId: string; sourceCampaignId: string; targetCampaignId: string } }
  | { type: 'MOVE_MAP_TO_CAMPAIGN'; data: { mapId: string; sourceCampaignId: string; targetCampaignId: string } };
...
export function buildCommandBody(commandType: CommandType, data: any): string | undefined {
  ...
  switch (commandType) {
    ...
    case 'CREATE_BATTLE':
      return JSON.stringify({ name: data.name, mode: data.mode, mapSize: data.mapSize, sceneDescription: data.sceneDescription });
    case 'MOVE_CREATURE':
...
export function createEmptyMap(width: number, height: number, description?: string): BattleMap {
  const cells: MapCell[][] = [];
...
export function isValidPosition(map: BattleMap, position: GridPosition): boolean {
  return position.x >= 0 && position.x < map.width &&
         position.y >= 0 && position.y < map.height;
}

// Get the map cell at a position
export function getMapCell(map: BattleMap, position: GridPosition): MapCell | null {
  ...
  for (let dy = 0; dy < gridSize; dy++) {
    for (let dx = 0; dx < gridSize; dx++) {
      ...
      if (!isValidPosition(map, checkPos)) {
        return { canOccupy: false, reason: 'Position extends beyond map bounds' };
      }
      ...
      if (!TERRAIN_INFO[cell.terrain].passable) {
        return { canOccupy: false, reason: `Blocked by ${cell.terrain.toLowerCase()}` };
      }
      ...
      if (existingCreatures) {
        const blockingCreature = existingCreatures.find(creature => {
          ...
          for (let ody = 0; ody < otherGridSize; ody++) {
            for (let odx = 0; odx < otherGridSize; odx++) {
              ...
              if (occupiedPos.x === checkPos.x && occupiedPos.y === checkPos.y) {
                return true;
              }
            }
          }
        ...
        if (blockingCreature) {
          return { canOccupy: false, reason: `Blocked by ${blockingCreature.name}` };
        }
      }
    }
  }

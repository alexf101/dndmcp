...
import type { BattleState, MapCell, TerrainType, Creature, CreatureSize } from '../types';
...
const MapCell = styled.div<{
  ...
  ${props => {
    switch (props.terrain) {
      ...
      case 'Door':
        return `background: ${props.isDoorOpen ? '#6c757d' : '#495057'}; color: white;`;
      case 'Window':
  ...
  ${props => props.hasCreature && props.isCreatureCenter && `
    border: 2px solid #007bff;
    background: linear-gradient(45deg, ${props.creatureSize === 'Large' || props.creatureSize === 'Huge' || props.creatureSize === 'Gargantuan'
      ? 'rgba(0, 123, 255, 0.3)'
      : 'rgba(0, 123, 255, 0.5)'}, transparent);
  `}
  ...
  .color-box {
    ...
    ${props => {
      switch (props.terrain) {
        ...
        case 'Door':
          return `background: ${props.isDoorOpen ? '#6c757d' : '#495057'};`;
        case 'Window':
...
export default function BattleMapVisualization({ battle }: BattleMapVisualizationProps) {
  if (!battle) {
    return (
      <MapContainer>
        <h3>🗺️ Battle Map</h3>
    ...
    if (creature.position) {
      ...
      for (let dx = 0; dx < gridSize; dx++) {
        for (let dy = 0; dy < gridSize; dy++) {
          const occupiedKey = `${creature.position.x + dx},${creature.position.y + dy}`;
  ...
  return (
    <MapContainer>
      <h3>🗺️ Battle Map ({map.width}×{map.height})</h3>
        ...
        {map.cells.flat().map((cell) => {
          ...
          return (
            ...
            >
              ...
              {cell.terrain === 'Wall' && '🧱'}
              {cell.terrain === 'Water' && '🌊'}
              {cell.terrain === 'Door' && (cell.doorOpen ? '🚪' : '🔒')}
              {cell.terrain === 'Stairs' && '📐'}
              {cell.terrain === 'Pit' && '🕳️'}
              ...
              {creature && isCreatureCenter && (
                <CreatureToken size={creature.size} isPlayer={creature.isPlayer}>
      ...
      <Legend>
        {Array.from(usedTerrains).map(terrain => (
          <LegendItem
            key={terrain}
            terrain={terrain}
            isDoorOpen={terrain === 'Door' ? Array.from(doorStates.values())[0] : undefined}
          >
        ...
        {battle.creatures.length > 0 && (
          <>
            <LegendItem terrain="Empty">
              <div className="color-box" style={{ background: '#28a745', borderRadius: '50%' }} />
              <span>Player Characters</span>
            </LegendItem>
            <LegendItem terrain="Empty">
              <div className="color-box" style={{ background: '#dc3545', borderRadius: '50%' }} />
